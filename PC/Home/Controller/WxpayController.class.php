...
class WxpayController extends Controller {
	...
	public function new_pay($order_id='000'){
		//全局引入微信支付类
		...
		{
			//触发微信返回code码
			$this->fromurl="http://qz.imweixi.com/Home/Wxpay/new_pay";
			$url = $jsApi->createOauthUrlForCode($this->fromurl);
			//http://qz.imweixi.com/index.php/Home/Wxpay/new_pay
			//var_dump($this->fromurl);exit;
			//https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx6944b1cc7de67483&redirect_uri=http://qz.imweixi.com/index.php/Home/Wxpay/new_pay&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect
			Header("Location: $url");
		}else
		{
			//获取code码，以获取openid
		...
		$unifiedOrder->setParameter("body","亲子支付");//商品描述
		//自定义订单号，此处仅作举例
		$timeStamp = time();
		//$out_trade_no = \WxPayConf_pub::APPID.$timeStamp;  
		//$out_trade_no = 123;
		//查询此项订单金额
		$unifiedOrder->setParameter("out_trade_no",18);//商户订单号
	...
	public function notify(){
		...
		//file_put_contents("log.txt","this is the 99\r\n",FILE_APPEND);
		if($notify->checkSign() == FALSE){
			$notify->setReturnParameter("return_code","FAIL");//返回状态码
			$notify->setReturnParameter("return_msg","签名失败");//返回信息
		}else{
			$data['status']=1;
			$oid=(int)$notify->data["out_trade_no"];
		    $ordermodule=M('order')->where('oid='.$oid)->save($data);
		...
		if($notify->checkSign() == TRUE)
		{
			...
			if ($notify->data["return_code"] == "FAIL") {
				...
				$this->redirect('Home/Min/pcpay_fail');//成功跳转
			}
			elseif($notify->data["result_code"] == "FAIL"){
				...
				$oid=(int)$notify->data["out_trade_no"];
				$this->orderhandle($oid);
				$this->redirect('Home/Min/pcpay_succ');//成功跳转
			}
			//商户自行增加处理流程,
			//例如：更新订单状态
			//例如：数据库操作
	...
	private function orderhandle($out_trade_no)
	{
		$data1['status']=1;
		$data1['pay_time']=time();
		$data1['pay_type']=2;//微信支付
		//修改订单状态为已支付
		$filter1['oid']=$out_trade_no;
		$res= M("order") -> where($filter1) ->save($data1);
		//减少用户的积分账户金额
		$ordermodel=M("order") -> where($filter1)->field('star_amount,uid,ac_id')->find();
		...
		//判断活动的报名人数
		$ac_id=$ordermodel['ac_id'];
		$entry_count=M('order')->where("ac_id=".$ac_id." and status=1")->count();
		$activemodel=M('active')->where('ac_id='.$ac_id)->field('partake_num')->find();
		if($entry_count==$activemodel['partake_num'])
		{
			$data3['status']=1;
			M('active')->where('ac_id='.$ac_id)->save($data3);
		}
	}
	...
	public function native_pays(){
        ...
        $unifiedOrder = new \UnifiedOrder_pub();
        $oid=$_GET['oid'];
        $ordemodel=M('order')->where('oid='.$oid)->find();
        $out_trade_no=$oid;
        $total_fee=($ordemodel['order_amount']-($ordemodel['star_amount']/300))*100;
        //设置统一支付接口参数
        //设置必填参数
        //appid已填,商户无需重复填写
        //mch_id已填,商户无需重复填写
        ...
        //spbill_create_ip已填,商户无需重复填写
        //sign已填,商户无需重复填写
        $unifiedOrder->setParameter("body","零距离童军订单支付");//商品描述
        //自定义订单号，此处仅作举例
        $timeStamp = time();
        $out_trade_no = \WxPayConf_pub::APPID.$timeStamp;
