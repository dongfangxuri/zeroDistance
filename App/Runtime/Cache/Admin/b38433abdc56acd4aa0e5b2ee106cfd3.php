...
<title>添加开通城市 - 活动管理 - 零距离童军</title>
<meta name="keywords" content="零距离童军">
<meta name="description" content="零距离童军">
</head>
<body>
<article class="page-container">
	<form action='/Admin/Active/city_add.html' method='post' class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">选择省份：</label>
			<div class="formControls col-xs-4 col-sm-2"> 
			  <span class="select-box">
				<select class="select" id="province">
                  <?php if(is_array($list)): foreach($list as $key=>$vo): ?><option value="<?php echo ($vo["areaid"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; ?>
				</select> 
				</span>
			</div>
		</div>
		<div class="row cl">
		  <label class="form-label col-xs-4 col-sm-2">选择城市：</label>
			<div class="formControls col-xs-4 col-sm-2">
			 <span class="select-box">
				<select id="citys" name="areaid">  
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">开通：</label>
			<div class="formControls col-xs-4 col-sm-2">
			  <span class="select-box">
				<select class="select" name="state">
					<option value="1" select="selected">开通</option>
...
$("#province").change(function() {  
    $.ajax({  
        type: "get",  
        url: "/Admin/Active/getcitylist", // type =2表示查询市  
        data: {"parentid": $(this).val(), "type": "2"},  
        dataType: "json",  
        success: function(data) {  
            $("#citys").html("<option value=''>请选择市</option>");  
            $.each(data, function(i, item) {  
                $("#citys").append("<option value='" + item.areaid + "'>" + item.name + "</option>");  
            });  
        }  
    });  
}); 
